...
class QuantumBlur(namedtuple('QuantumBlur', ('h', 'w', 'alpha'))):
    ...
    This transforms (3 x n x m) data i.e. image data using Wooton et al's Quantum Blur: arXiv:2112.01646.
    
    Note: due to the normalising and unnormalising, this method is currently restricted to the cifar10 dataset.
    ...
    def __call__(self, x: np.ndarray, x0: int, y0: int) -> np.ndarray:
        ...
        x0 , y0 : int
            Corner such that (x0, y0), (x0 + W, y0), (x0 + W, y0 + H), (x0, y0 + H) is
            a valid rectangle
    ...
    def options(self, shape: Tuple[int]) -> List[Dict[str, int]]:
        ...
        i.e. return pairs such that (x0, y0), (x0 + W, y0), (x0 + W, y0 + H), (x0, y0 + H) make
        a valid rectangle in the image coordinates.  

        Parameters
        ----------
        shape : Tuple[int]
            Shape of the image tensor expected to be either (1 x n x m) or (3 x m x n)
...
class GaussianBlur(namedtuple('GaussianBlur', ('h', 'w'))):
    ...
    This transforms (3 x n x m) data i.e. image data using a normal distribution with 
