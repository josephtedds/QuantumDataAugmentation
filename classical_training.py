...
from blur_transform import QuantumBlur, GaussianBlur
...
def pre_process_data() -> Tuple[DataLoader, DataLoader]:
    ...
    # Pad the image with numpy reflective padding then apply the other transforms to training set.
...
def train_model(train_set, valid_set, epochs = 20, n_runs = 3, train_transforms = [Crop(32, 32), FlipLR(), Cutout(8, 8)], save_log_path = None):
    lr_schedule = PiecewiseLinear([0, 5, epochs], [0, 0.4, 0])
    batch_size = 512

    # Apply cropping + random flipping and random cutout to the training set
...
def train_quantum_augmentations():
    ...
    all_pairs = [(i_alpha, j_blur_size) for i_alpha in alphas for j_blur_size in blur_size]
