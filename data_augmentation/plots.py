...
def classical_comparison_plot(show_plots=True, save=False, run: int = 0):
    ...
    for i_transform in TRANSFORMS:
        ...
        experiments_df = pd.concat(
            [experiments_df, i_experiment_df], ignore_index=True
    ...
    for i_metric in METRICS:
        i_ax = sn.lineplot(
            data=experiments_df, x="epoch", y=i_metric, hue="Transform"
        )
        i_ax.set_xlim((0, None))

        i_y_label = (
            prettify_labels(i_metric)
        ...
        if save:
            plt.savefig(
                rf"figures/training_performance_run_{run}_{i_metric}.png",
                bbox_inches="tight",
            )
...
def quantum_blur_hyperparameter(
    show_plots: bool = True, save: bool = False, run: int = 0
):
    set_cc_colours()
    ...
    for i_experiment in os.listdir("experiment_runs"):
        ...
        with open(rf"experiment_runs/{i_experiment}") as i_f:
            i_experiment_df = pd.read_csv(i_f)
    ...
    for i_metric in METRICS:
        i_ax = sn.lineplot(
            data=experiments_df.sort_values("Hyperparameters"),
            x="epoch",
            y=i_metric,
            hue="Hyperparameters",
        )
        ...
        if save:
            plt.savefig(
                rf"figures/hyperparam_opt_run{run}_{i_metric}.png",
                bbox_inches="tight",
            )
...
def show_classical_data_augmentation(
    show_images: bool = True, save: bool = False
):
    DATA_DIR = "../data"
    ...
    if save:
        plt.savefig(
            r"figures/classical_aug_translate.png", bbox_inches="tight"
        )
    ...
    if save:
        plt.savefig(
            r"figures/classical_aug_gaussian_blur.png", bbox_inches="tight"
        )
...
def show_quantum_blur(show_images: bool = True, save: bool = False):
    ...
    train_set = list(zip(*preprocess(dataset["train"], transforms).values()))
    transformed_train_set = Transform(
        train_set, [QuantumBlur(8, 8, 0.1, False)]
    )
    transformed_train_set.set_random_choices()
    plt.imshow(colours_channel_last(transformed_train_set.__getitem__(0)[0]))
    plt.axis("off")
