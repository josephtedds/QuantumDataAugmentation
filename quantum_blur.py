...
from myrtle_core import transpose, preprocess, pad, Transform, Crop, FlipLR, Cutout
...
def _image2heights(image: np.ndarray):
    ...
    """
    _,Lx,Ly = image.shape
    heights = [{(x, y): image[j,x,y] for x in range(Lx) for y in range(Ly)}  for j in range(3)]
    return heights
...
def _heights2image(heights):
    ...
    for i, i_heights in enumerate(heights):
        ...
        for x in range(Lx):
            for y in range(Ly):
                if (x,y) in i_heights:
                    image[i, x, y] = int(255* float(i_heights[x,y])/i_h_max)
                else:
                    image[i, x, y] = 0
    return image
...
if __name__ == "__main__":
    ...
    img1_transformed = colours_channel_last(transformed_train_set.__getitem__(0)[0])
    img1_blurred = colours_channel_last(blur_image(colours_channel_first(img1)))
